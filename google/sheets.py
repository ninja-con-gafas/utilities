...
def get_credentials(key: str, scopes: List[str]) -> Credentials:
    """
    Retrieves Google Service Account credentials from a JSON key file and assigns the provided scopes.

    args:
        key (str): The file path to the service account key JSON file.
    ...
    raises:
        None
    """
    return Credentials.from_service_account_file(filename=key, scopes=scopes)
